...
**Exercised by:** benchmark — the `+deep research` cell in `BeirDeepResearchTests`, driven by `DeepResearchAblationRow` through a real `AddRagNet` pipeline with `UseDeepResearch`, measured in Phase 6.2.1 and pinned in `BeirReproduction`. A real `gpt-4o-mini` judges each of SciFact's 300 judged queries sufficient or not and writes sub-queries when not; 657 calls, cached, and a replay reproduces the figure. **nDCG@10 0.70219**, Recall@10 0.82622, MRR@10 0.66642 — against the Real dense control's **0.67742**, so **+0.02477, the second-largest gain any technique has had on SciFact**, behind HyDE's +0.03647. The loop provably ran: 184 of 300 queries expanded past the control's page, 73,727 chunks added. **MRR rose more than Recall (+0.02885 against +0.01300)**, so the chunks the sub-queries pull in land high rather than merely landing — a technique that only widened the candidate set would have moved Recall alone. **Read it as a LARGER SEARCH, not a better ranker:** this cell retrieves up to ten times per query where the control retrieves once, and the gain is bought with retrieval work and model calls. **What this does NOT establish:** the page is not capped to `TopK` — the largest returned 1,260 against 250, 5.04×, issue #475 — so the two sides differ in page size as well as in technique, and that page's ordering mixes scores taken against different query vectors, making it not internally comparable; SciFact only, no second corpus; and no answer-quality figure, only retrieval. **The allowlist entry asked for "an answer-harness arm" and that route was wrong** — `DeepResearchRetriever` is an `IRetriever` decorator, not an `IAnswerEngine`, so it takes the retrieval-cell route its siblings take. The run is opt-in behind `RAGNET_BEIR_LONG_RUNS` and `RAGNET_DEEP_RESEARCH_GENERATE`.
...
**Exercised by:** benchmark — the two arms of `BeirMetadataExtractionTests`, driving the real `LlmMetadataExtractionBehavior` with a real `gpt-4o-mini` over real corpora, measured in Phase 6.2.1. A one-key schema asks for each chunk's subject **domain**, so the corpus it came from is the ground truth and accuracy is measurable rather than asserted — asking for "topic" or "keywords" produces values nothing can check, and the cell would then report extraction as working on the strength of the JSON parsing alone. **SciFact, whole corpus: 20,155 chunks, 98.79% coverage, 99.88% of extracted values correct.** **FiQA, capped at 1,000 as the control: 62.70% coverage, 96.33% correct.** The arms differ in the corpus and in nothing else — same behaviour, model, schema, temperature and cache — so the **36.09-point coverage gap is a property of the corpus, not of the behaviour**, which is what the pilot's 120 chunks could suggest and not establish. **The shortfall is the finding, not a caveat on it:** the misses are the model returning a literal `{}`, nothing throws and nothing fails to parse, and `LlmMetadataExtractionBehavior` attaches with `TryAdd` and logs a per-chunk warning — so 37.30% of a FiQA-shaped corpus comes out unlabelled with nothing louder than the log to say so, and a filter over that key quietly does not match those chunks. Both coverage figures are pinned in the cell at ±0.5 points and mutation-checked at 0.6; replay is deterministic, so they pin the **shipped attachment path** rather than the model. **What this does NOT establish:** no retrieval-quality figure — whether extracted metadata improves recall is unmeasured and this cell structurally cannot say; the section's **Q&A-pair route is untouched**, only the structured-tag route ran; and FiQA's rate carries an n of 1,000, its full 121,236 units being ~$28 and ~44 hours. The run is opt-in behind `RAGNET_BEIR_LONG_RUNS` and `RAGNET_METADATA_EXTRACTION_GENERATE`.
...
**Exercised by:** benchmark — `BeirMindMapTests` runs the shipped `MindMapExtractor` with a real `gpt-4o-mini` over the sixty real news articles of `MultiHopRagSlice`, measured in Phase 6.2.1. **60 of 60 articles produced a titled root carrying children; 0 came back empty. 1,071 nodes in total, deepest tree 4 levels.** 60 calls, cached, and a replay reproduces it in under a second. **The guard is the substance here, because this extractor cannot fail loudly:** `ExtractAsync` returns `EmptyRoot()` — `("", "", [])` — on an LLM failure *and* on an unparseable reply, and its own summary says it never throws, so a run where every call failed returns sixty empty roots and reports as a completed extraction. `AssertTheModelActuallyBuiltTrees` refuses that. **What this does NOT establish:** no quality figure and no control — a mind map has no qrels, and the entry asked for an exercise rather than a score; nothing checks that a tree is a *good* summary of its article. **`MaxDepth` is not enforced** — it is interpolated into the prompt and the parsed tree is never checked against it, so a model that ignores the instruction produces a deeper tree silently; this run's deepest is 4 node-levels against a documented maximum of 3, which is at the limit under edge-counting and over it under node-counting, and the option does not say which it means. The graph-store persistence path is not exercised: the cell passes `graphStore: null`. The run is opt-in behind `RAGNET_MIND_MAP_GENERATE`.
...
**Exercised by:** benchmark — `ConversationMemoryExerciseTests` runs twenty ten-turn conversations, built from MultiHop-RAG's own judged questions and article text, through the shipped `ConversationMemoryPipeline` resolved from a real `AddRagNet` container with a real `gpt-4o-mini` writing the summaries; measured in Phase 6.2.1. It processes after **every** turn, which is what an answer engine does, so the window slides ten times per conversation rather than once. **200 histories processed, 140 came back carrying a summary message, 1,120 messages fell out of the window.** 140 calls against the 200 priced — the first three turns of each conversation trim nothing, so no summary is requested. **The guard is the substance:** `GenerateSummaryAsync` catches every exception and returns `null`, and `ProcessAsync` then simply omits the summary message, so a run that reached no model at all returns correctly-trimmed histories and is otherwise indistinguishable from success. `AssertTheModelActuallySummarised` refuses that. **What this does NOT establish:** no quality figure — nothing checks that a summary is faithful to what it replaced, and no downstream answer is scored with and without it; `MaxTokens` budgeting is not exercised (this cell drives `MaxExchanges`); and `PersistentConversationMemory`'s cross-session recall is a separate decorator this cell does not register. The run is opt-in behind `RAGNET_CONVERSATION_MEMORY_GENERATE`.
...
**Exercised by:** benchmark — the `RealLateChunking` cell in `BeirAblationTests`, measured on three corpora and pinned in `BeirReproduction`: SciFact **0.65510** (−0.02232 against the Real dense control), FiQA **0.38369** (+0.02800), ArguAna **0.48988** (+0.01429), each reproduced by a second run agreeing to five decimals. **This is the first retrieval-quality figure late chunking has ever had** — the entry that owed this cell said it was "measured once in Phase 3.7 and never pinned", and 3.7 measured SciFact's parity dense anchor instead. **Late chunking is anti-correlated with the other measured techniques**, the only one negative on SciFact and positive on FiQA and ArguAna. **Caveat carried with every figure:** the cell varies boundaries *and* embedding — it windows at its own 256 tokens rather than reusing `RecursiveChunkingStrategy` — so no figure here isolates whole-document context. Measuring it also exposed the `MaxTokens` defect that had been silently degrading `UseLateChunking()` to ordinary embeddings on long documents (#452).
...
**Status:** ✅ Done. `HydeOptions.HypothesisCount` (default 3) and `HypothesisTemperature` (default 0.8) drive `IHypotheticalDocumentGenerator.GenerateManyAsync`; `HydeBehavior` embeds all hypotheses in one batch, mean-pools + L2-normalizes, and passes the averaged vector downstream via the internal `RetrievalOptions.EmbeddingOverride` (consumed by the vector-store and ensemble dense arms, bypassing the embedding cache). Partial generation failures are tolerated; total failure falls back to the plain query. Cost per retrieval: `n` LLM calls + `n` embedding inputs (one batch call).
**Exercised by:** test — `HydePipelineParityTests` holds the shipped `HydeBehavior` to the harness's `HydeAblationRow`: the same three hypotheses over the same store, through a real `AddRagNet` pipeline with `UseHyde` on one side and the row's own `BuildSearchVector` on the other, asserting identical ids, scores and order. Fast tier, no model and no corpus. **This is what makes the phase's three HyDE figures statements about the shipped code** — SciFact +0.03647, FiQA −0.00886, ArguAna −0.02053 against each corpus's Real dense control — rather than about a re-implementation of it. Mutation-checked: skewing one component of the shipped pooled vector by 5% fails it at rank 0 on a score difference of 0.0002. It does **not** measure HyDE or exercise the generator's prompt; generation is faked precisely so that pooling, embedding and search are the only things compared.
